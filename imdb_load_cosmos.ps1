...
dotnet run --project src/altgraph_data_app imdb_bulk_load_movies imdb_graph > out/imdb_bulk_load_movies.txt

# SELECT count(1) FROM c where c.doctype = 'movie' --> 380,570

echo "========================"
echo "imdb_bulk_load_people..."
dotnet run --project src/altgraph_data_app imdb_bulk_load_people imdb_graph > out/imdb_bulk_load_people.txt
...
# dotnet run --project src/altgraph_data_app imdb_bulk_load_small_triples imdb_graph > out/imdb_bulk_load_small_triples.txt

# SELECT count(1) FROM c where c.doctype = 'sm_triple' --> 3,914,263

echo "============================="
echo "imdb_bulk_load_movies_seed..."
dotnet run --project src/altgraph_data_app imdb_bulk_load_movies_seed imdb_seed > out/imdb_bulk_load_movies_seed.txt
